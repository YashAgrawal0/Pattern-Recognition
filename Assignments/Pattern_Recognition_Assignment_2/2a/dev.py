...
# MEAN,COVARIANCE,PI,data_points = kMeans("Dsadad")
...
def plotContour(mu, sigma, color):
	# print(mu)
	# print(sigma)
	plt.plot([mu[0]], [mu[1]], marker='o', markersize=3, color="black")
	...
	a=w       #radius on the x-axis
	b=h      #radius on the y-axis
	t_rot=theta #rotation angle
	...
	u=mu[0]       #x-position of the center
	v=mu[1]      #y-position of the center
	a=w/2.0       #radius on the x-axis
	b=h/2.0     #radius on the y-axis
	t_rot=theta #rotation angle
	...
	u=mu[0]       #x-position of the center
	v=mu[1]      #y-position of the center
	a=w/10.0       #radius on the x-axis
	b=h/10.0     #radius on the y-axis
	t_rot=theta #rotation angle
...
DATA_POINTS = []
for i in range(3):
	mean, cov, pi, data_points = kMeans("train_class"+str(i+1)+".txt")
...
for i in range(3):
	data_points, dim = input("test_class"+str(i+1)+".txt")
	l[i] = len(data_points)
