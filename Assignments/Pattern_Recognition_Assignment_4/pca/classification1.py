...
#====================================================================================================================================
def input_all_data(fileName):
	data_points = []
	
	for j in range(3):
		# f = open("2c/group06/Train/feature/"+str(j),"r")
		f = open(fileName+"train_class"+str(j+1)+".txt","r")
		d = 0

		for i in f.readlines():
			x = [float(jj) for jj in i.split()]
...
# 	# plt.savefig("Log_vs_Itr.png",bbox_inches="tight", pad_inches=0.5)
# 	# plt.show()
# 	return mean,covariance,pi,data_points
	

def GMM(mean,covariance,pi,data_points,cl):
	l_theta_old = 1000000000000
	l_theta = 0.0
	itr = 1
	iterationsX,loglikely = [],[]
...
def contour_plot(mu, cov, ax, color):
...
def findClass(data, MEAN, COVARIANCE, PI):
	ll = np.zeros(3)
	# data = [x, y]
	# print(data)
	for c in range(3):
...
DATA_POINTS = []
for i in range(3):
	mean, cov, pi, data_points = kMeans("reduced_data/"+str(l_red)+"/train_class"+str(i+1)+".txt")
	# mean_gmm, cov_gmm, pi_gmm, data_points_gmm = kMeans("train_class"+str(i+1)+".txt")
	mean_gmm, cov_gmm, pi_gmm, data_points_gmm = GMM(mean, cov, pi, data_points,i)
...
# for j in range(k):
# 	contour_plot(MEAN[0][j], COVARIANCE[0][j], ax,  "red")
# for j in range(k):
# 	contour_plot(MEAN[1][j], COVARIANCE[1][j], ax, "green")
# for j in range(k):
# 	contour_plot(MEAN[2][j], COVARIANCE[2][j], ax, "blue")
...
for i in range(3):
	data_points, dim = input("reduced_data/"+str(l_red)+"/test_class"+str(i+1)+".txt")
	l[i] = len(data_points)
	for j in range(len(data_points)):
		c = findClass(data_points[j], MEAN, COVARIANCE, PI)
...
# 	while(j<=6):
#		point = [i,j]
# 		c = findClass(point, MEAN, COVARIANCE, PI)
# 		if(c == 0):
# 			back1X.append(i)
# 			back1Y.append(j)
# 		elif(c==1):
